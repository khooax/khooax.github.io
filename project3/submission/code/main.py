...
def computeH(im1_pts, im2_pts):
    """
    im1_pts: (n x 2) array of points in im1
    im2_pts: (n x 2) array of corresponding points in im2
    H: (3 x 3) matrix
    """
    n = im1_pts.shape[0]
    A = []
    ...
    print(np.round(H, 3))   # rounded for readability
    print()
...
def apply_homography(H, pts):
    ...
    H: (3 x 3) matrix
    pts: (n,2) array of 2D points [(x,y), ...]
...
def warpImageBilinear(im, H, fill_value=0, out_shape=None):
    """
    Returns (warped_image, alpha_mask).
    """
    im = np.asarray(im)
    src_h, src_w = im.shape[:2]
    channels = im.shape[2]
...
def get_panorama_bounding_box(images, homographies):
    """
    Compute bounding box for the panorama given images and homographies to a common reference.
    images: list of np.ndarray images (each shape (h,w,3))
    homographies: list of 3x3 homographies mappign to ref image
